...
4. 历史对比与连贯性检查 (Historical Comparison): 
   ...
   - **追踪报道**: 若某话题在历史上已出现且今日有重大突破或实质进展，应设为高优先级，并在 reasoning 中注明“延续自历史专题：[名称]”。
   - **冗余拦截**: 若今日资讯仅是历史话题的简单重复（如：无新进展的常规更新、已报道过的旧闻），请将其分配为 FLASH_NEWS 或 DISCARD。
5. 识别今日最重大的 1-3 个“核心焦点”，优先考虑用户关注点。
# 任务分发逻辑
...
5. 链接溯源: 在提到关键信息时，保留原始链接。**必须确保正文引用的 URL 与对应的文章内容严格匹配，严禁张冠李戴。**
6. 参考链接: 在文章末尾列出所有参考链接。**链接的标题必须与其 URL 指向的实际网页标题完全一致，且必须优先从【核心资讯】中提取。**
...
1. **CRITICAL (红线错误 - 必须拦截并重写)**:
   ...
   - 虚构不存在的实体或事件（真正的幻觉）。
   - 信源混淆: 参考链接标题与 URL 内容不符（例如标题是路透社，链接是华尔街见闻）。
...
- **参考链接**: 检查文章末尾的参考链接是否与文章内容一致，链接是否正确。
