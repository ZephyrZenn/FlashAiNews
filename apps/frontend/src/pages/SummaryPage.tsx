...
const SummaryPage = () => {
  ...
  if (selectedBrief) {
    ...
    return (
      <Layout showBackButton onBackClick={() => setSelectedBrief(null)}>
        <div className="h-full p-4 flex items-center justify-center bg-slate-100/30">
          <div className={`w-full max-w-7xl h-full flex flex-col bg-white shadow-2xl rounded-sm overflow-hidden relative ${cardStyle.color}`}>
            <div className="p-12 pb-6 border-b border-black/5 mx-12 shrink-0">
            ...
            <div className="flex-1 flex overflow-hidden relative">
              ...
              {headings.length > 0 && showOutline && (
                <div className="w-64 border-l border-black/5 bg-white/70 backdrop-blur-sm shrink-0">
                  <div className="sticky top-0 p-6 max-h-full overflow-y-auto custom-scrollbar">
              ...
              {headings.length > 0 && !showOutline && (
                <button
                  ...
                  className="absolute right-0 top-1/2 -translate-y-1/2 -translate-x-4 w-10 h-20 bg-white/90 backdrop-blur-sm border border-black/10 rounded-l-lg shadow-lg flex flex-col items-center justify-center gap-1 text-slate-400 hover:text-slate-700 hover:bg-white transition-all z-10"
                  title="显示大纲"
                >
                  <List size={18} />
                  <span className="text-[10px] font-medium">显示</span>
                </button>
              )}
            </div>
          </div>
  ...
  if (!displayBriefs || displayBriefs.length === 0) {
    ...
    return (
      <Layout>
        <div className="h-full p-12 custom-scrollbar overflow-y-auto">
          <div className="max-w-7xl mx-auto">
            ...
            <div className="flex flex-col items-center justify-center text-center pt-12">
              <div className="relative mb-8">
                ...
                <div className="relative p-8 bg-white border border-slate-100 rounded-[2.5rem] shadow-xl">
                  <FileText size={48} className="text-indigo-600" />
                </div>
              </div>
              <h2 className="text-3xl font-black text-slate-800 mb-3">{emptyMessage}</h2>
            ...
            {displayBriefs.map((brief, index) => {
              ...
              return (
                <div
                  ...
                  className={`group relative p-8 cursor-pointer transition-all duration-500 hover:scale-[1.03] ${cardStyle.color} ${cardStyle.rotation} rounded-sm shadow-sm hover:shadow-2xl h-[340px] flex flex-col border border-black/5`}
                >
                  {/* Paper clip effect */}
                  ...
                  {/* Header */}
                  <div className="flex justify-between items-center mb-6 text-[10px] font-black uppercase tracking-widest opacity-40">
                    <span className="truncate">{groupTitle}</span>
                    <span>{formatDate(brief.pubDate)}</span>
                  </div>
                  
                  {/* Key points */}
                  <ul className="space-y-4 mb-8 text-[15px] leading-relaxed text-slate-800 font-medium flex-1 overflow-hidden">
